    ...
    def run(self) -> Result:
        ...
        try:
            ...
            if self.cfg.protocol == Protocol.UDP:
                ...
                if getattr(self.cfg, "rate", None) is not None:
                    rate_bps = int(self.cfg.rate)
                    # some libs expose iperf_set_test_bitrate, others expose
                    # a more generic iperf_set_test_rate; accept either.
                    fn = _try_set("iperf_set_test_bitrate")
                    if fn is None:
                        fn = _try_set("iperf_set_test_rate")
                    if fn is None:
                        raise UnsupportedFeatureError("This lib lacks 'iperf_set_test_bitrate' or 'iperf_set_test_rate'")
