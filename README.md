...
- Ready for `uv` workflows, ruff/black/mypy/pytest
...
```bash
export PY_IPERF3_LIB=/usr/local/lib/libiperf.so
```
...
- Initial release: cffi (ABI) wrapper of libiperf (client & server),
  Pydantic models, async support, ruff/black/mypy/pytest, CI with uv.
